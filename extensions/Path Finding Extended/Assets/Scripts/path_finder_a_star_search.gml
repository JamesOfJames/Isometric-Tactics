...
while (!ds_priority_empty(frontier))
    ...
    var NeighbourList = path_finder_grid_neighbours(Graph,path_finder_grid_cell_get_x(Graph,current),path_finder_grid_cell_get_y(Graph,current)) ;
    var Size = ds_list_size(NeighbourList) ;
    //show_debug_message("Neighbours : " + string(Size)) ;
   for(i=0;i<Size;i++)
        { 
        var next = ds_list_find_value(NeighbourList,i) ;
        var new_cost = ds_map_find_value(cost_so_far,current) + path_finder_grid_cell_get_weight(Weighted,next) ;
